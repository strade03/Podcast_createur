...
void WaveformWidget::setFullWaveform(QSharedPointer<QVector<qint16>> waveformPtr)
{
    fullWaveform = waveformPtr; // ✅ Partage la mémoire (pas de copie !)
    
    if (fullWaveform && !fullWaveform.isNull()) {
        totalSamples = fullWaveform->size(); // ✅ CORRECTION ICI
    } else {
        totalSamples = 0;
    }
    
    isLoaded = (totalSamples > 0);
    
    // Corrections des positions si dépassement
...
{
    fullWaveform.reset(); // ✅ Libère le pointeur partagé
    
    displayWaveform.clear();
    displayWaveform.squeeze();
...
void WaveformWidget::recalcCache()
{
    ...
    if (fullWaveform.isNull() || fullWaveform->isEmpty()) { // ✅ CORRECTION
        cacheValid = true;
        return;
    }
    ...
    if (samplesPerPixel > 200) { 
        step = static_cast<int>(samplesPerPixel / 100); 
        if (step < 1) step = 1;
    }

    qint64 totalSize = fullWaveform->size(); // ✅ CORRECTION
    const qint16* rawData = fullWaveform->constData(); // ✅ CORRECTION

    for (int x = 0; x < w; ++x) {
        qint64 startIdx = static_cast<qint64>((x + offsetPixels) * samplesPerPixel);
        qint64 endIdx = static_cast<qint64>((x + offsetPixels + 1) * samplesPerPixel);

        if (startIdx >= totalSize) break;
        if (endIdx > totalSize) endIdx = totalSize;

        qint16 maxValInt = 0;
        for (qint64 i = startIdx; i < endIdx; i += step) {
            qint16 val = std::abs(rawData[i]); 
            if (val > maxValInt) maxValInt = val;
        }
        displayWaveform[x] = maxValInt / 32768.0f;
    }
...
void WaveformWidget::scrollToPixel(int x) {
    ...
    int totalWidth = fullWaveform.isNull() ? 0 : static_cast<int>(fullWaveform->size() / samplesPerPixel);
